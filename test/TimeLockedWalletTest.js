...
const BiconomyToken = artifacts.require("./BiconomyToken.sol");
...
contract('TimeLockedWallet', (accounts) => {
    ...
    it("Owner can withdraw the BiconomyToken after the unlock date", async () => {
        //set unlock date in unix epoch to now
        let now = Math.floor((new Date).getTime() / 1000);
        //create the wallet contract 
        let timeLockedWallet = await TimeLockedWallet.new(creator, owner, now);

        //create BiconomyToken contract
        let biconomyToken = await BiconomyToken.new({ from: creator });
        //check contract initiated well and has 1M of tokens
        assert(1000000000000 == await biconomyToken.balanceOf(creator));

        //load the wallet with some Biconomy tokens
        let amountOfTokens = 1000000000;
        await biconomyToken.transfer(timeLockedWallet.address, amountOfTokens, { from: creator });
        //check that timeLockedWallet has BiconomyTokens
        assert(amountOfTokens == await biconomyToken.balanceOf(timeLockedWallet.address));
        //now withdraw tokens
        await timeLockedWallet.withdrawTokens(biconomyToken.address, { from: owner });
        //check the balance is correct
        let balance = await biconomyToken.balanceOf(owner);
